    ...
    <div class="py-8 max-w-7xl mx-auto sm:px-6 lg:px-8 text-slate-100">
        ...
        <div
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 text-sm text-slate-300">
                    ...
                    <div>
                        ...
                        <span @class([
                            ...
                            'bg-slate-500/20 text-slate-200 border-slate-400/30' => !in_array(
                                $audit->state,
                    ...
                    <div class="md:col-span-3">
                        ...
                        <p class="text-slate-100">{{ $audit->audit_range }}</p>
                    </div>
                </div>
            </div>
            ...
            <div id="findingsSection"
                class="flex flex-col md:flex-row justify-between items-center p-6 border-y border-slate-800/70 text-slate-200 gap-4">
            ...
            <div class="p-6 space-y-6">
                ...
                <form action="{{ route('quality.audits.findings.store', $audit) }}" method="POST"
                    class="bg-slate-900/70 p-6 rounded-xl border border-slate-800/70">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                        ...
                        <div class="md:col-span-2">
                            <label for="description" class="block text-sm font-semibold text-slate-300 mb-1">Descripción del
                                Hallazgo</label>
                        ...
                        <div>
                            <label for="classification"
                                class="block text-sm font-semibold text-slate-300 mb-1">Clasificación</label>
                        ...
                        <div>
                            ...
                            <select name="severity" id="severity" required
                                ...
                                <option value="medium" {{ old('severity', 'medium') == 'medium' ? 'selected' : '' }}>Media
                                </option>
                        ...
                        <div>
                            <label for="discovery_date" class="block text-sm font-semibold text-slate-300 mb-1">Fecha de
                                Descubrimiento</label>
                            <input type="date" name="discovery_date" id="discovery_date"
                                value="{{ old('discovery_date', date('Y-m-d')) }}" required
                                class="block w-full bg-slate-800/60 border border-slate-700 text-slate-100 rounded-lg shadow-sm py-2 px-3 focus:border-sky-500 focus:ring focus:ring-sky-500 focus:ring-opacity-50 text-sm">
                        ...
                        <div>
                            <label for="evidence" class="block text-sm font-semibold text-slate-300 mb-1">Evidencia
                                (Opcional)</label>
                    ...
                    <div class="overflow-x-auto border border-slate-800/70 rounded-lg">
                        <table class="min-w-full divide-y divide-slate-800/70 text-sm">
                            ...
                            <tbody id="findingsTableBody"
                                class="bg-transparent divide-y divide-slate-800/70 text-slate-300">
                                @forelse ($audit->findings as $finding)
                                    <tr class="hover:bg-slate-900/60 transition">
                                        <td class="px-6 py-4 text-slate-100 whitespace-normal">{{ $finding->description }}
                                        </td>
                                        <td class="px-6 py-4">
                                            <span @class([
                                                'px-3 py-1 inline-flex text-xs font-semibold rounded-full border',
                                        ...
                                        <td class="px-6 py-4">
                                            {{ $finding->discovery_date ? \Carbon\Carbon::parse($finding->discovery_date)->format('d/m/Y') : '-' }}
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center space-x-4 text-xs">
                                                ...
                                                <form action="{{ route('quality.findings.destroy', $finding) }}"
                                ...
                                @empty
                                    <tr class="no-results-row"> {{-- Fila especial --}}
                                        <td colspan="4" class="px-6 py-6 text-center text-slate-500">Aún no se han
                                            registrado hallazgos.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
            ...
            {{-- Cabecera con Búsqueda --}}
            <div id="correctiveActionsSection"
                class="flex flex-col md:flex-row justify-between items-center p-6 border-y border-slate-800/70 text-slate-200 gap-4">
            ...
            <div class="p-6 space-y-6">
                ...
                @if ($audit->findings->isNotEmpty())
                    <form action="" method="POST" id="correctiveActionForm"
                        class="bg-slate-900/70 p-6 rounded-xl border border-slate-800/70 mt-6">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4">
                            ...
                            <div>
                                <label for="finding_id" class="block text-sm font-semibold text-slate-300 mb-1">Asignar al
                                    Hallazgo:</label>
                                <select name="finding_id" id="finding_id" required
                                    class="block w-full bg-slate-800/60 border border-slate-700 text-slate-100 rounded-lg shadow-sm py-2 px-3 focus:border-sky-500 focus:ring focus:ring-sky-500 focus:ring-opacity-50 text-sm">
                                    @foreach ($audit->findings as $finding)
                                        <option value="{{ $finding->id }}">{{ Str::limit($finding->description, 70) }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="md:col-span-2">
                                <label for="action_description"
                                    class="block text-sm font-semibold text-slate-300 mb-1">Descripción de la
                                    Acción</label>
                                <input type="text" name="description" id="action_description" required
                                    class="block w-full bg-slate-800/60 border border-slate-700 text-slate-100 rounded-lg shadow-sm py-2 px-3 focus:border-sky-500 focus:ring focus:ring-sky-500 focus:ring-opacity-50 text-sm">
                                @error('description')
                                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label for="user_id"
                                    class="block text-sm font-semibold text-slate-300 mb-1">Responsable</label>
                            ...
                            <div>
                                <label for="due_date" class="block text-sm font-semibold text-slate-300 mb-1">Fecha
                                    Límite</label>
                        ...
                        <tbody id="actionsTableBody" class="bg-transparent divide-y divide-slate-800/70 text-slate-300">
                            ...
                            @forelse ($audit->findings as $finding)
                                @forelse ($finding->correctiveActions as $action)
                                    ...
                                    <tr class="hover:bg-slate-900/60 transition">
                                        @if ($loop->first)
                                            <td class="px-6 py-4 align-top text-slate-400 italic"
                                                rowspan="{{ $loop->count }}">
                                                {{ Str::limit($finding->description, 40) }}
                                            </td>
                                        @endif
                                        <td class="px-6 py-4 text-slate-100 whitespace-normal">{{ $action->description }}
                                        </td>
                                        <td class="px-6 py-4 text-slate-100">
                                            {{ $action->responsible->full_name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center space-x-3 text-xs">
                                                ...
                                                <form action="{{ route('quality.corrective-actions.destroy', $action) }}"
                                                    method="POST" class="m-0 p-0">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (form && findingSelect) {
                function updateFormAction() {
                    ...
                    const baseUrl =
                        "{{ route('quality.findings.corrective-actions.store', ['finding' => ':id']) }}";
            ...
            function setupLiveSearch(inputId, tableBodyId, searchCellsIndices, colspan) {
                ...
                searchInput.addEventListener('input', function() {
                    ...
                    const originalDoc = parser.parseFromString(
                        `<table><tbody>${originalRowsHTML}</tbody></table>`, 'text/html');
                    ...
                    originalRows.forEach(row => {
                        ...
                        for (const index of searchCellsIndices) {
                            ...
                            const cellText = (row.cells[index]?.querySelector('span')
                                    ?.textContent || row.cells[index]?.textContent || '')
                                .toLowerCase();
                    ...
                    if (!found) {
                        ...
                        noResultRow.innerHTML =
                            `<td colspan="${colspan}" class="px-6 py-6 text-center text-slate-500">No se encontraron registros que coincidan con "${this.value}".</td>`;
