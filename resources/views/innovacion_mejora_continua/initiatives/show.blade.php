    ...
    <div class="container mx-auto px-4 py-8 text-slate-100">
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <div class="lg:col-span-2 space-y-6">
                <div
                    ...
                    <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                        <div class="flex-1">
                            ...
                            <div class="flex flex-wrap items-center gap-3 text-xs">
                                <span
                                    class="inline-flex items-center rounded-full border px-3 py-1 font-semibold {{ $statusPills[$initiative->status] ?? 'bg-slate-500/20 text-slate-200 border-slate-400/30' }}">
                        ...
                        <div class="flex flex-wrap gap-3">
                            ...
                            <form action="{{ route('innovacion-mejora-continua.initiatives.destroy', $initiative) }}"
                                method="POST" onsubmit="return confirm('¿Eliminar esta iniciativa?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                    ...
                    <div class="mt-6 space-y-6 text-sm leading-relaxed text-slate-300">
                        ...
                        @if ($initiative->description)
                            <div>
                                <h3 class="text-xs font-semibold uppercase tracking-wide text-slate-400">Descripción
                                    completa</h3>
                                <div
                                    class="mt-2 rounded-2xl border border-slate-800/60 bg-slate-900/60 p-4 text-slate-200 whitespace-pre-line">
                ...
                <div
                    ...
                    <div class="mt-6 space-y-4">
                                ...
                                <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                    <div>
                                        <div class="flex flex-wrap items-center gap-3 text-sm text-slate-400">
                                            <span
                                                class="inline-flex items-center gap-2 rounded-full border border-amber-400/30 bg-amber-500/10 px-3 py-1 text-amber-200">
                                            ...
                                            <span class="text-slate-500">Evaluado por <strong
                                                    class="text-slate-300">{{ $evaluation->evaluator->name }}</strong></span>
                                        </div>
                        ...
                        @empty
                            <div
                                class="rounded-2xl border border-slate-800/60 bg-slate-950/60 p-10 text-center text-sm text-slate-400">
            ...
            <div class="space-y-6">
                <div
                    ...
                    <div class="space-y-4 text-sm text-slate-300">
                        <div>
                            ...
                            <p class="mt-1">{{ $initiative->responsibleUser->full_name ?? 'Sin asignar' }}</p>
                        </div>
                        @if ($initiative->responsibleTeam)
                            <div>
                                <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Equipo responsable
                                </p>
                                <p class="mt-1">{{ $initiative->responsibleTeam->full_name }}</p>
                            </div>
                        @endif
                ...
                <div
                    class="rounded-3xl border border-slate-800/70 bg-slate-950/70 p-4 text-xs text-slate-500 shadow-xl shadow-slate-950/40">
                    <p><span class="font-semibold text-slate-300">Creada:</span>
                        {{ $initiative->created_at->format('d/m/Y H:i') }}</p>
                    <p><span class="font-semibold text-slate-300">Actualizada:</span>
                        {{ $initiative->updated_at->format('d/m/Y H:i') }}</p>
