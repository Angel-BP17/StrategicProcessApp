    ...
    <div class="container mx-auto px-4 py-8 text-slate-100">
        ...
        <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-slate-400">Administra ideas, responsables, impactos esperados y recursos necesarios.
                </p>
            </div>
        ...
        <div
            class="mb-6 rounded-3xl border border-slate-800/70 bg-slate-950/70 p-6 shadow-xl shadow-slate-950/40 backdrop-blur">
            <form method="GET" action="{{ route('innovacion-mejora-continua.initiatives.index') }}"
                ...
                <div>
                    ...
                    <select name="responsible_user_id"
                        ...
                        @foreach (\App\Models\User::orderBy('full_name')->get() as $user)
                            <option value="{{ $user->id }}" @selected(request('responsible_user_id') == $user->id)>{{ $user->full_name }}</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div class="grid gap-6">
            @forelse ($initiatives as $initiative)
                <div
                    class="rounded-3xl border border-slate-800/70 bg-slate-950/70 shadow-xl shadow-slate-950/40 transition hover:-translate-y-1 hover:shadow-2xl">
                    <div class="p-6">
                        <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                            <div class="flex-1 min-w-0">
                                <div class="flex flex-wrap items-start gap-3">
                                    <a href="{{ route('innovacion-mejora-continua.initiatives.show', $initiative) }}"
                                        class="text-lg font-semibold text-white hover:text-sky-300 transition">
                                        {{ $initiative->title }}
                                    </a>
                                    @php
                                        $statusPills = [
                                            ...
                                            'implementada' =>
                                                'bg-fuchsia-500/10 text-fuchsia-200 border-fuchsia-400/30',
                                            'cerrada' => 'bg-slate-500/20 text-slate-200 border-slate-400/30',
                                        ];
                                    @endphp
                                    <span
                                        class="inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold {{ $statusPills[$initiative->status] ?? 'bg-slate-500/20 text-slate-200 border-slate-400/30' }}">
                                ...
                                <div class="mt-4 flex flex-wrap gap-4 text-xs text-slate-500">
                                    <span class="inline-flex items-center gap-2">
                                        <svg class="size-3.5 text-slate-500" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                        </svg>
                                        <span class="font-semibold text-slate-300">Responsable:</span>
                                    ...
                                    @if ($initiative->responsibleTeam)
                                        <span class="inline-flex items-center gap-2">
                                            ...
                                            <span class="font-semibold text-slate-300">Equipo:</span>
                                    ...
                                    <span class="inline-flex items-center gap-2">
                                        ...
                                        <span class="font-semibold text-slate-300">Inicio:</span>
                                    ...
                                    <span class="inline-flex items-center gap-2">
                                        ...
                                        <span class="font-semibold text-slate-300">Evaluaciones:</span>
                        ...
                        @if ($initiative->estimated_impact)
                            <div
                                class="mt-5 rounded-2xl border border-sky-500/30 bg-sky-500/10 px-4 py-3 text-sm text-sky-100">
                                <span class="font-semibold">Impacto estimado:</span> {{ $initiative->estimated_impact }}
                            </div>
                        @endif
                    </div>
                    <div
                        class="flex items-center justify-between border-t border-slate-800/60 bg-slate-950/60 px-6 py-3 text-xs text-slate-500">
                        <span>Creada {{ $initiative->created_at->diffForHumans() }}</span>
                        <div class="flex gap-3">
                            <a href="{{ route('innovacion-mejora-continua.initiatives.show', $initiative) }}"
                                class="inline-flex items-center gap-1 text-sky-300 hover:text-sky-100">
                                Ver detalles
                            ...
                            <a href="{{ route('innovacion-mejora-continua.initiatives.edit', $initiative) }}"
                                class="inline-flex items-center gap-1 text-slate-300 hover:text-slate-100">
                                <svg class="size-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @empty
                <div
                    class="rounded-3xl border border-slate-800/70 bg-slate-950/70 p-12 text-center shadow-xl shadow-slate-950/40">
                    <svg class="mx-auto mb-4 size-16 text-slate-600" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="flex flex-wrap justify-center gap-3">
                        @if (request()->hasAny(['search', 'status', 'responsible_user_id']))
                            <a href="{{ route('innovacion-mejora-continua.initiatives.index') }}"
                                class="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-700/70 px-5 py-2.5 text-sm font-semibold text-slate-300 transition hover:border-slate-500 hover:text-slate-100">Limpiar
                                filtros</a>
                        @endif
                        <a href="{{ route('innovacion-mejora-continua.initiatives.create') }}"
