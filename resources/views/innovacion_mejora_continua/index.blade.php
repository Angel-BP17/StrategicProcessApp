...
<div class="container mx-auto px-4 py-8 text-slate-100">
    ...
    <div class="mb-8">
        ...
        <p class="text-sm text-slate-400">Sistema de gestión de iniciativas de innovación, implementación de nuevas metodologías y evaluación de mejoras aplicadas</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <!-- Nueva Iniciativa -->
        <a href="{{ route('innovacion-mejora-continua.initiatives.create') }}" class="bg-gradient-to-br from-sky-500 via-blue-600 to-indigo-600 rounded-2xl shadow-lg shadow-sky-500/40 p-6 text-white hover:shadow-2xl hover:-translate-y-1 transition-all duration-200">
        ...
        <a href="{{ route('innovacion-mejora-continua.initiatives.index') }}" class="bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500 rounded-2xl shadow-lg shadow-emerald-500/40 p-6 text-white hover:shadow-2xl hover:-translate-y-1 transition-all duration-200">
            <div class="flex items-center justify-between mb-4">
        ...
        <!-- Dashboard -->
        <a href="{{ route('innovacion-mejora-continua.dashboards.index') }}" class="bg-gradient-to-br from-fuchsia-500 via-purple-500 to-violet-500 rounded-2xl shadow-lg shadow-fuchsia-500/40 p-6 text-white hover:shadow-2xl hover:-translate-y-1 transition-all duration-200">
            <div class="flex items-center justify-between mb-4">
            ...
            @foreach($statusConfig as $status => $config)
                <div class="rounded-2xl p-4 text-center backdrop-blur {{ $config['wrapper'] }}">
                    <div class="flex justify-center mb-2">
                        <svg class="w-8 h-8 {{ $config['icon'] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-slate-950/60 border border-slate-800/70 rounded-2xl shadow-xl shadow-slate-900/50 p-6">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('innovacion-mejora-continua.initiatives.index') }}" class="text-sky-300 hover:text-sky-200 text-sm font-medium">
                    Ver todas →
                </a>
            </div>
            ...
            @empty
                <div class="text-center py-8 text-slate-500">
                    <svg class="mx-auto h-12 w-12 text-slate-600 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"/>
                    </svg>
        ...
        <div class="bg-slate-950/60 border border-slate-800/70 rounded-2xl shadow-xl shadow-slate-900/50 p-6">
                    ...
                    <div class="flex justify-between items-start gap-3">
                        <div class="flex-1 min-w-0">
                            <h4 class="font-semibold text-white">
                                <a href="{{ route('innovacion-mejora-continua.initiatives.show', $evaluation->initiative) }}" class="hover:text-sky-200 transition">
                        ...
                        <div class="ml-2 flex items-center text-amber-300 gap-1">
                            ...
                            <span class="text-lg font-semibold text-white">{{ number_format($evaluation->score, 1) }}</span>
                        </div>
                    </div>
                </div>
            @empty
            ...
            @if($averageScore)
                <div class="mt-4 pt-4 border-t border-slate-800/70">
                    <div class="flex items-center justify-between bg-amber-500/10 border border-amber-400/30 rounded-xl p-3">
