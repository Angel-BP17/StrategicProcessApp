...
<div class="container mx-auto px-4">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between mb-8">
        ...
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('planning.plans.show', ['plan' => $objective->plan_id]) }}"
                class="inline-flex items-center gap-2 rounded-2xl border border-slate-700/60 px-4 py-2 text-xs sm:text-sm font-semibold text-slate-300 transition hover:border-slate-500 hover:text-slate-100">←
                Volver al plan</a>
            @can('objective.manage')
            <a href="{{ route('planning.objectives.edit', ['plan' => $objective->plan_id, 'objective' => $objective->id]) }}"
                class="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 px-5 py-2 text-xs sm:text-sm font-semibold text-white shadow-lg shadow-emerald-500/30 transition hover:-translate-y-0.5">Editar
                objetivo</a>
            @endcan
        </div>
    </div>
    ...
    <div
        class="mb-10 rounded-3xl border border-slate-800/70 bg-slate-950/70 p-6 shadow-2xl shadow-slate-950/40 backdrop-blur">
        <h2 class="text-lg font-semibold text-slate-100">Descripción</h2>
        <p class="mt-3 text-sm leading-relaxed text-slate-300">
            {{ $objective->description ?? 'Este objetivo aún no cuenta con una descripción detallada.' }}</p>
        <div class="mt-4 flex flex-wrap items-center gap-4 text-xs font-medium text-slate-400">
            <span
                class="inline-flex items-center gap-2 rounded-full border border-emerald-400/30 bg-emerald-500/10 px-3 py-1 text-emerald-200">
                Meta: {{ $objective->goal_value ?? '—' }}
            </span>
            <span
                class="inline-flex items-center gap-2 rounded-full border border-sky-400/30 bg-sky-500/10 px-3 py-1 text-sky-200">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-4">
        ...
        @can('objective.manage')
        <a href="{{ route('planning.kpis.create', ['plan' => $plan, 'objective' => $objective]) }}"
            class="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-sky-500 via-indigo-500 to-fuchsia-500 px-5 py-2 text-xs sm:text-sm font-semibold text-white shadow-lg shadow-indigo-500/40 transition hover:-translate-y-0.5">Nuevo
            KPI</a>
        @endcan
    </div>
    ...
    <div
        class="rounded-3xl border border-slate-800/60 bg-slate-950/60 p-10 text-center shadow-inner shadow-slate-950/40">
        ...
        <div class="rounded-3xl border border-slate-800/60 bg-slate-950/60 p-5 shadow-xl shadow-slate-950/40">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-4">
                <div>
                    ...
                    <div class="text-xs text-slate-500">Meta: {{ $kpi->target_value ?? '—' }}
                        {{ $kpi->unit }}</div>
                ...
                <div class="flex flex-wrap gap-2 text-xs font-semibold">
                    ...
                    <form method="POST"
                        ...
                        @method('DELETE')
                        <button
                            class="rounded-full border border-rose-400/40 px-3 py-1 text-rose-200 hover:border-rose-300 hover:text-rose-100">Eliminar</button>
                    </form>
                    @endcan
                </div>
            </div>
